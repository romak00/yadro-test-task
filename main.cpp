...
public:
    ...
    Time(const int minutes) {
        int h = 0, m = 0;
        h = minutes / 60; m = minutes % 60;
        time_string = (h < 10 ? "0" + std::to_string(h) : std::to_string(h)) + ":" + (m < 10 ? "0" + std::to_string(m) : std::to_string(m));
    }
...
public:
    ...
    std::deque<std::string> client_queue;                                     //(0,n > 0) - за столом, второе число - номер стола
    ...
    void action_3_client_is_waiting(const Time& time, const std::string& client_name) {
        ...
        else if (client_list_with_tables[client_name].second > 0) {                                         /// если уже сидит за столом
            throw ActionError(time.time_string, "AlreadyGaming");
        }
...
int main(int argc, char** argv) {
    ...
    secondary_output << closing_time_str << '\n';
    main_output << secondary_output.rdbuf();
    std::cout << main_output.rdbuf();

    int i = 0;
    for (auto& table : club.tables) {
        std::cout << ++i << " " << price_per_hour * table.hours_rounded_up << " " << Time(table.time_sat_in_minutes).time_string << '\n';
    }
    fin.close();
    return 0;
}